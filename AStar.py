...
class State:
    def __init__(self, position, prev, cost, f):
...
def heuristic(pos, goals):
    min = inf
    for g in goals:
        req = max(abs(pos[0] - g[0]), abs(pos[1] - g[1]))
        if req < min:
            min = req
    return min
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    pq = []
    reached = {}
    initial = State(own_pieces[0][1], None, 0, heuristic(own_pieces[0][1], goals))
    heapq.heappush(pq, initial)
    reached[initial.position] = 0
    while pq:
        curr = heapq.heappop(pq)
        if curr.isGoal(goals):
            ans = curr.getPath()
            return ans, curr.cost
        for a in curr.getLegalMoves(rows, cols, board.grid):
            successor = State(a, curr, curr.cost+grid[a[0]][a[1]], curr.cost+grid[a[0]][a[1]]+heuristic(a, goals))
            if successor.position not in reached.keys() or successor.cost < reached[successor.position]:
                heapq.heappush(pq, successor)
                reached[successor.position] = successor.cost
    return [], 0
#############################################################################
