...
class State:
    def __init__(self, position, prev, cost):
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    pq = []
    reached = {}
    initial = State(own_pieces[0][1], None, 0)
    heapq.heappush(pq, initial)
    reached[initial.position] = 0
    while pq:
        curr = heapq.heappop(pq)
        if curr.isGoal(goals): #must be late goal test for UCS
            ans = curr.getPath()
            return ans, curr.cost
        for a in curr.getLegalMoves(rows, cols, board.grid):
            successor = State(a, curr, curr.cost+grid[a[0]][a[1]])
